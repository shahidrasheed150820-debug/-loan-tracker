...
function render() {
  ...
  $('#loanList').innerHTML=shown.map(loan=>{const f=loanFigures(loan), overdue=isOverdue(loan), pct=f.total?Math.min(100,(f.collected/f.total)*100):0;return `
    ...
    <div class="amount-row"><div><span>Total due</span><strong>${money(f.total)}</strong></div><div><span>Balance</span><strong class="balance">${money(f.balance)}</strong></div></div><div class="progress" title="${Math.round(pct)}% collected"><span style="width:${pct}%"></span></div>
    ...
    <div class="card-foot"><p>${loan.rate}% flat interest · ${loan.payments.length} payment${loan.payments.length===1?'':'s'}</p><button class="text-button edit" data-id="${loan.id}">View & edit</button></div></article>`}).join('');
}
...
function updatePreview(){const f=loanFigures({amount:$('#amount').value,rate:$('#rate').value,payments:[]});$('#previewInterest').textContent=money(f.interest);$('#previewTotal').textContent=money(f.total)}
...
$('#closeDialog').addEventListener('click',()=>dialog.close()); $('#cancelDialog').addEventListener('click',()=>dialog.close()); $('#searchInput').addEventListener('input',render); $('#amount').addEventListener('input',updatePreview); $('#rate').addEventListener('input',updatePreview);
